...
async def waiting(bot: hikari.GatewayBot, ctx: tanjun.abc.SlashContext, message: hikari.Message) -> hikari.InteractionCreateEvent:
    try:
        event = await bot.wait_for(hikari.InteractionCreateEvent, 60, lambda x: x.interaction.message.id == message.id and x.interaction.user.id == ctx.author.id)
...
async def edit_logs(ctx: tanjun.abc.SlashContext, bot: hikari.GatewayBot, db: DB, mes: hikari.Message) -> None:
    ...
    try:
        ...
        row.add_button(hikari.ButtonStyle.PRIMARY, "1").set_label("Logs").add_to_container()
        row.add_button(hikari.ButtonStyle.SUCCESS, "01").set_label("Color").add_to_container()
    except KeyError:
        text = f"I haven't found logs in server, would you like to set up them?"
        row.add_button(hikari.ButtonStyle.SUCCESS, "1").set_label("Continue").add_to_container()
    row.add_button(hikari.ButtonStyle.SECONDARY, "0").set_label("Cancel").add_to_container()
    ...
    if res.interaction.custom_id == "0":
        ...
        row = ctx.rest.build_action_row()
        row.add_button(hikari.ButtonStyle.PRIMARY, "is").set_label("Colored").add_to_container()
        row.add_button(hikari.ButtonStyle.SECONDARY, "not").set_label("Uncolored").add_to_container()
...
@tanjun.with_author_permission_check(8, error_message="You should have Administrator to do it")
@tanjun.as_slash_command("settings", "manage setting on your server", default_to_ephemeral=True)
async def settings(ctx: tanjun.abc.SlashContext, bot: hikari.GatewayBot = tanjun.injected(type=hikari.GatewayBot)) -> None:
    db = DB()
    row = ctx.rest.build_action_row()
    ...
    menu.add_to_container()
    mes = await ctx.respond("High time to change smth!", component=row, ensure_result=True)
    res = await waiting(bot, ctx, mes)
